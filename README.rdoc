...
A check in represents a user's visit to a spot, and will typically have a counterpart in other location-based applications such as Foursquare, Gowalla, and Facebook Places.
    ...
    Whatser.client.check_ins.create( spot_id, params[:check_in] )
    m = Whatser.check_ins.follows.new( params[:check_in] )
    m.save
...
When a user connects a social network account to the Whatser API, their friends are automatically imported at this time. To keep a user's friends updated, you can call a get_friends method on the Twitter and Facebook classes to import any new friends of that user who have recently registered with Whatser.
...
Copyright (c) 2010 Travis Dunn / Whatser Company. See LICENSE.txt for
further details.
